...
date = '190404_'
ksz = 2 # Options: 0,1,2
ksz_name = ['','reionKSZ_','bothKSZ_']

# i/o directories
out_dir = "output/"+date+expf_name+"_"+str(gradCut)+"_"+ksz_name[ksz]  # for plots
...
# Read config
iniFile = "input/recon_astro2020.ini"
...
if ksz:
    if ksz == 1:
        ells_ksz,ksz_power_1d = np.loadtxt('data/reion_ksz_extrapolate.txt',unpack=True)
    else:
        ells_ksz,ksz_power_1d = np.loadtxt('data/both_ksz_extrapolate.txt',unpack=True)

    ksz_power_1d = ksz_power_1d/(2.7255e6)**2 # fix units
        
    ksz_power_2d_dat = fmaps.interp(ells_ksz,ksz_power_1d)(modlmap_dat)
    ksz_power_2d_sim = fmaps.interp(ells_ksz,ksz_power_1d)(modlmap_sim)
    kszgen = enmap.MapGen(shape_sim,wcs_sim,ksz_power_2d_sim[None,None])
    print shape_sim,wcs_sim,ksz_power_2d_sim.shape
...
with io.nostdout():
    if ksz:
        qest = Estimator(template_dat,
                     ...
                     noiseX2dTEB=[nT+ksz_power_2d_dat,nP,nP],
                     noiseY2dTEB=[nT+ksz_power_2d_dat,nP,nP],
...
for index in my_tasks:
    ...
    # add kSZ to lensed sim
    if ksz:
        kszmap = kszgen.get_map(seed=(1,1,1,index))
        print 'ksz map shape: ',kszmap.shape
        lensed = lensed + kszmap
